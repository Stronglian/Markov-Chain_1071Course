...
#%%
def TrainAllAndPrint(trainData, testData, parmSet = None, field_names = ["pi", "A", "B"], boolTablePrint = True, trainDataOrg = None):
    """ 跑單一次的測試流程
    """
    ...
    if trainDataOrg is None:
        trainDataOrg = trainData
    #建模
    ...
    result_train_rate = HMM_B.Test(trainDataOrg, boolPrint=False)
    #建表
...
def ReverseData(inputDataSet):
    outputDataSet = np.zeros_like(inputDataSet)
    for i in range(inputDataSet.shape[0]):
        outputDataSet[i,:,:] = inputDataSet[i,:,::-1]
    print()
...
#%% 全測試_反向 - 2018/11/27
    # 取用 上次資料，將其反向
    trainData_rever = ReverseData(trainData)
#    raise ValueError
    testData_rever = ReverseData(testData)
    # 顯示資料
    print("TrainData:", trainDataName, "TestData:", testDataName, "BUT reverse")
    print("Training Rate:","(use",trainDataName,")")
    print("Testing Rate:","(use",testDataName,")")
    # RUN
    for i, parmSet in enumerate(parmSetList):
...
#        TrainAllAndPrint(trainData.copy(), trainData.copy(), parmSet)
        TrainAllAndPrint(trainData_rever.copy(), testData_rever.copy(), parmSet)#, trainDatOrg=trainData.copy())
    print("\n\n", "="*50, "\n")
    
#%% 收尾
    __endTime = time.time()
    print('\n\n\nEND,', 'It takes', __endTime-__startTime ,'sec.')  
