...
class HidenMarkovModel_genaral():
    def __init__(self):
        ...
        self.stateOutput = [str(i) for i in range(1, len(self.probabilityMatrix[0,:])+1)]
        return
...
#        tempRan = random.randrange(0, self.randomNum)
        tempRan = np.random.randint(self.randomNum)
    ...
    def CalOneRound(self, repeatTimes=5):
        ...
        #重複
        for t in range(1, repeatTimes):#因為第一次已經跑了
